...
def make_message(title, sub, contents, contents_type='context'):
    message = '> :llama: **' + title + '**\n\n**' + sub + '**\n'
...
async def help(ctx):
    ...
    message = make_message('사용 가능한 명령어 목록', '',
                           contents, 'command')
...
async def crazy_error(ctx, error):
    ...
    if '50007' in str(error):
        message = make_message('에구구', '',
                               '테러를 받은 유저가 저를 차단했어요...칫...:put_litter_in_its_place:')

        await ctx.send(message)
    else:
        message = make_message('명령어 \"테러\" 사용법', '멈출 수 없으니 신중히 사용하세요!',
                               '!테러 @<유저명> <반복 횟수(최대20)>', 'command')
